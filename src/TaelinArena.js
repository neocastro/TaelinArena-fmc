if (typeof window !== "undefined") {
  var TA = require("./TaelinArena.fmc.js");
...
function render_game({game, canvox, canhud, cam}) {
  ...
  var hero_pos = TA["TaelinArena.Game.get_position_by_pid"](0, game);

  // Renders each game thing
  // TA.map_stage(thing=>render_thing(thing,hud,lights))(game);
  TA["TaelinArena.Game.map_stage"](thing=>render_thing(thing,hud,lights))(game);
...
function execute_command(inp, game) {
  ...
  if (inp.input === "SDIR") {
    ...
    let d = v3 => v3(x)(y)(0);
    cmd = TA["TaelinArena.Game.Command.new"](inp.player)(TA["TaelinArena.Game.Input.sdir"](d));
    ...
    var f = null;
    switch (inp.input) {
      case "KEY0": keyx = TA["TaelinArena.Game.Input.key0"]; break;
      case "KEY1": keyx = TA["TaelinArena.Game.Input.key1"]; break;
      case "KEY2": keyx = TA["TaelinArena.Game.Input.key2"]; break;
      case "KEY3": keyx = TA["TaelinArena.Game.Input.key3"]; break;
      case "KEY4": keyx = TA["TaelinArena.Game.Input.key4"]; break;
      case "KEY5": keyx = TA["TaelinArena.Game.Input.key5"]; break;
    }
    cmd = TA["TaelinArena.Game.Command.new"](inp.player)(keyx(p));
  }
  return TA["TaelinArena.exec_command"](cmd)(game);
}

function make_thing([name, {pid,sid,pos,nam}]) {
  var thing;
  name = name.toLowerCase();
  thing = TA["TaelinArena.Game.Thing.new"];
  if (TA[name+"_fun"]) {
    thing = TA["TaelinArena.Game.set_thing_fun"](thing)(TA[name+"_fun"]);
  }
  if (sid !== undefined) {
    thing = TA["TaelinArena.Game.set_thing_sid"](thing)(sid);
  }
  if (pid !== undefined) {
    thing = TA["TaelinArena.Game.set_thing_pid"](thing)(pid);
  }
  if (pos !== undefined) {
    thing = TA["TaelinArena.Game.set_thing_pos"](thing)(v3 => v3(pos.x)(pos.y)(pos.z));
  }
  if (nam !== undefined) {
    thing = TA["TaelinArena.Game.set_thing_nam"](thing)(string_to_sstring(nam)); 
  }
  return thing;
};
...
function GameRunner(gid, things) {
  var self = {};
  self.gid = gid;
  self.state = TA["TaelinArena.Game.Game.new"](array_to_slist(things.map(make_thing)));
  self.turns = [];

  function turn() {
    if (self.state) {
      self.state = TA["TaelinArena.exec_turn"](self.state);
    }
  }
